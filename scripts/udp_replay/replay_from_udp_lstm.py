...
def parse_arguments(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "When enabled, steer towards predicted future positions generated by the LSTM predictor"
        ),
    )
    parser.add_argument(
        "--lstm-history",
        type=int,
        default=10,
        help="Number of past observations retained by the online LSTM predictor",
    )
    parser.add_argument(
        "--lstm-horizon",
        type=int,
        default=5,
        help="Number of future steps to predict for each observation",
    )
    parser.add_argument(
        "--lstm-log-interval",
        type=int,
        default=50,
        help="Log LSTM prediction loss every N observations with available predictions",
    )
    parser.add_argument(
        "--timing-output",
...
class EntityManager:
    ...
    def get_record(self, object_id: str) -> Optional[EntityRecord]:
        return self._entities.get(object_id)

    def update_predicted_target(
        self, object_id: str, target: Optional[carla.Location]
    ) -> None:
        record = self._entities.get(object_id)
        if record is not None:
            record.predicted_target = target
    ...
    def step_all(self, dt: float) -> None:
        ...
        for record in self._entities.values():
            ...
            if self._use_lstm_target and record.predicted_target is not None:
                target = record.predicted_target
                        ...
                        try:
                            ...
                            for raw_message in decode_messages(payload):
                                ...
                                if applied and lstm_predictor is not None:
                                    ...
                                    if prediction.log_loss and prediction.loss is not None:
                                        LOGGER.info(
                                            "LSTM prediction loss for %s: %.3f",
                                            state.object_id,
                                            prediction.loss,
                                        )
                                    if args.use_lstm_target:
                                        manager.update_predicted_target(
                                            state.object_id, prediction.predicted_target
                                        )
