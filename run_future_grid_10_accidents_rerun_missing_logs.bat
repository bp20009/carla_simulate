...
REM Usage:
REM   run_future_grid_10_accidents_rerun_missing_logs.bat [SUMMARY_IN] [CSV_PATH] [OUTDIR]
REM Defaults:
...
set "SUMMARY_OUT=%OUTDIR%\summary_rerun_missing_logs.csv"
echo accident_idx,accident_payload_frame_ref,accident_tag,method,lead_sec,rep,seed,switch_payload_frame,ran_ok,accident_after_switch,first_accident_payload_frame,status,source_summary,source_status,missing_reason> "%SUMMARY_OUT%"
...
for /f "usebackq skip=1 tokens=1-12 delims=," %%a in ("%SUMMARY_IN%") do (
  ...
  if not exist "!RUN_LOGS!" set "MISSING_REASON=!MISSING_REASON!logs_missing;"
  call :append_missing_reason "!RUN_META!" "meta_missing_or_empty" MISSING_REASON
  call :append_missing_reason "!RUN_COLL!" "collisions_missing_or_empty" MISSING_REASON
  call :append_missing_reason "!RUN_ACTOR!" "actor_missing_or_empty" MISSING_REASON
  call :append_missing_reason "!RUN_IDMAP!" "id_map_missing_or_empty" MISSING_REASON

  if defined MISSING_REASON (
    set /a "MISSING_ROWS+=1"
    echo ---------------------------------------------------------
    echo [RERUN !MISSING_ROWS!] accident=!ACCIDENT_PF! tag=!ACC_TAG! method=!METHOD! lead=!LEAD! rep=!REP! seed=!SEED!
    ...
    echo !ACC_IDX!,!ACCIDENT_PF!,!ACC_TAG!,!METHOD!,!LEAD!,!REP!,!SEED!,!SWITCH_PF!,!RAN_OK!,!AFTER_SWITCH!,!FIRST_ACC_PF!,!STATUS!,!SUMMARY_IN!,!SRC_STATUS!,!MISSING_REASON!>> "%SUMMARY_OUT%"
    if "!RAN_OK!"=="1" (
      set /a "RERUN_OK+=1"
    ) else (
      set /a "RERUN_FAIL+=1"
...
echo Missing rows detected: %MISSING_ROWS%
